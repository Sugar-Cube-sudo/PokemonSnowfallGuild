...
type SettingsView = 'main' | 'users' | 'approvals' | 'security' | 'system' | 'forum';
...
export default function SystemSettings({ onClose }: SystemSettingsProps) {
  ...
  const canManageForum = hasPermission(state.user, Permission.FORUM_MODERATE) ||
                        hasPermission(state.user, Permission.FORUM_MANAGE_POSTS) ||
                        hasPermission(state.user, Permission.FORUM_MANAGE_ACTIVITIES) ||
                        hasPermission(state.user, Permission.FORUM_CREATE_ACTIVITIES);
  ...
  const menuItems = [
    ...
    {
      id: 'forum' as SettingsView,
      title: '论坛管理',
      description: '审核帖子、管理活动和论坛内容',
      icon: MessageSquare,
      color: 'from-indigo-500 to-blue-600',
      available: canManageForum,
      stats: '待审核帖子'
    },
    {
  ...
  const renderCurrentView = () => {
    switch (currentView) {
      ...
      case 'forum':
        return <ForumManagement onClose={() => setCurrentView('main')} />;
      case 'approvals':
            ...
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-3">
                ...
                <UserAvatar 
                  ...
                  className="border-2 border-white/30 hover:border-white/50 transition-colors"
                />
              </div>
              <motion.button
